...
pub fn create_ingredient(conn: &Connection, ingredient: &Ingredient) -> Result<(), Error> {
    conn.execute(
        ...
        params![
            ...
            ingredient.calories,
            ingredient.carbohydrates_g,
            ingredient.fat_g,
            ingredient.protein_g,
            ingredient.saturated_fat_g,
            ingredient.polyunsaturated_fat_g,
            ingredient.monounsaturated_fat_g,
            ingredient.trans_fat_g,
            ingredient.fiber_mg,
            ingredient.sugar_mg,
...
pub fn get_all_ingredients(conn: &Connection) -> Result<Vec<Ingredient>, Error> {
    ...
    .query_map([], |row| {
        Ok(Ingredient {
            ...
            calories: row.get(5)?,
            carbohydrates_g: row.get(6)?,
            fat_g: row.get(7)?,
            protein_g: row.get(8)?,
            saturated_fat_g: row.get(9)?,
            polyunsaturated_fat_g: row.get(10)?,
            monounsaturated_fat_g: row.get(11)?,
            trans_fat_g: row.get(12)?,
            fiber_mg: row.get(13)?,
            sugar_mg: row.get(14)?,
